<template>
  <div>
    <Modal v-model:modelValue="showModalNuevo">
      <RegisterUserView @on-register="onRegister($event)" />
    </Modal>
    <Modal v-model:modelValue="showModalEdit">
      <EditUserView @on-update="onUpdate($event)" :item="itemToEdit" />
    </Modal>
    <h1>Users</h1>
...
export default {
  ...
  methods: {
    ...
    getList() {
      const vm = this;
      this.axios.get(this.baseUrl + "/users?_embed=categories&q=" + this.textToSearch)
      ...
      if (confirm("Are you sure you want to delete this user?")) {
        ...
        this.axios.delete(this.baseUrl + "/users/" + id)
          .then(function (response) {
            console.log(response);
            vm.getList();
            vm.$toast.show("User deleted.", "danger");
          })
          .catch(function (error) {
            console.error(error);
          });
    ...
    onUpdate(event) {
      ...
      this.$toast.show('Updated successfully', 'info');
    },
